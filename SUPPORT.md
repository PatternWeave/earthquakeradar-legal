# Earthquake Radar Support

Earthquake Radar is a mix-study app for **iPhone and iPad**. Import a screen recording or audio file. The first 30 seconds are fingerprinted on this device against the JVIZ Earthquake Weather archive.

## Help

- **No recording** — tap Study a vault sample. That scores a bundled archive window so you can see matches without importing a file.
- **Photos is empty** — pick a screen recording, or use Choose file and import from Files / the share sheet.
- **Study is slow the first time** — the vault index loads once on this device. Later studies reuse it.
- **Clip will not play** — playback uses HTTPS to jviz.net. Study still works offline. Only the one-minute match clip needs a network.
- **Wrong mix** — drag Frequency occurrence potential. Low values demand the same occupancy. High values open timing, drops, and wobble.

## Contact

Email [aataves@icloud.com](mailto:aataves@icloud.com).

Privacy policy: [index.html](index.html).
